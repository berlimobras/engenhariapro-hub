...
export default function Planos() {
  ...
  const handleSubscribe = async (planName: string) => {
    ...
    try {
      if (!user) {
        toast.error('Você precisa fazer login ou se cadastrar primeiro.');
        navigate('/register');
        return;
      }
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          >
            <Button 
              ...
              className="bg-accent hover:bg-accent/90 text-white px-8 py-6 text-lg rounded-xl font-bold shadow-[0_0_40px_-10px_rgba(var(--accent),0.5)] transition-all hover:scale-105"
            >
